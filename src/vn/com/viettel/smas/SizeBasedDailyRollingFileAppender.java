...
public class SizeBasedDailyRollingFileAppender extends FileAppender {
  ...
  private int maxReserveDays=Integer.MAX_VALUE;
  ...
  void rollOver() throws IOException {
    ...
    if (result) {
      ...
      if(this.maxReserveDays!=Integer.MAX_VALUE){
        this.deleteOldFiles();
      }
    } else {
  ...
  private void deleteOldFiles(){
    ...
    for(File f:file.getParentFile().listFiles()){
      String fn=f.getName();
      if(!fn.startsWith(shortFileName)) continue;//it's not mine logs!!!!
      
      int lastIndex=fn.lastIndexOf(".");
      if(lastIndex==-1) continue; //not mine log name format!
      if(!fn.substring(lastIndex+1).matches("\\d+")) continue; //not endwiths digits
      String dateStr=fn.substring(shortFileName.length(), lastIndex);
      Date fileDate = null;
      try {
        fileDate = this.sdf.parse(dateStr);
      } catch (ParseException e) {
        continue;
      }
      Date curDate = null;
      try {
        curDate=this.sdf.parse(sdf.format(now));//format date like 1970-01-01 00:00:00
      } catch (ParseException e) {
        continue;
      }
      long daydiff=(curDate.getTime()-fileDate.getTime())/(24 * 60 * 60 * 1000);
      if(daydiff > this.maxReserveDays){
        if(f.exists()){
          f.delete();
        }
      }
